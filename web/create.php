...
<html>
	<head>
		...
		<div id="forum">
			...
			<div id="content">
				<form action="create_topic.php" method="post" name="vytvor-temu">
					...
					<br>
					<button type='button' class='button' id="b" tabindex=-1><b>tučné</b></button>
					<button type='button' class='button' id="i" tabindex=-1><i>kurzíva</i></button>
					<button type='button' class='button' id="u" tabindex=-1><u>podčiarknuté</u></button>
					<button type='button' class='button' id="del" tabindex=-1><s>prečiarknuté</s></button>
		...
		<script>
		var _ftdata = { title: document.cookies.get('-newtopic-saved-title'), post: document.cookies.get('-newtopic-saved-post') };
		_ftdata['title'] && (document.forms['vytvor-temu'].elements['topic_title'].value = _ftdata['title']);
		_ftdata.post && (document.forms['vytvor-temu'].elements['prispevok'].innerHTML = _ftdata.post);
		
		document.forms['vytvor-temu'].onsubmit = function(e) {

			var DELETED = document.cookies.DELETED;
			document.cookies.set('-newtopic-saved-title', DELETED, -60 * 60 * 24); // deleted a day ago
			document.cookies.set('-newtopic-saved-post', DELETED, -60 * 60 * 24); // similarly
